...
class _MT5ExitMultiPositionFIFOConfirmedStub(_MT5BaseStub):
    """Two positions for same symbol; lowest ticket (FIFO) should be resolved."""
...
class _MT5ExitMultiPositionInvalidTicketStub(_MT5BaseStub):
    """Two positions for same symbol; one has unreadable ticket → fail closed."""
...
class _MT5ExitMultiPositionDuplicateTicketStub(_MT5BaseStub):
    """Two positions for same symbol with same ticket → fail closed."""
...
class TestExecutionGateSemantics(unittest.TestCase):
    ...
    # -- multi-position EXIT resolution tests --

    def test_exit_multi_position_resolves_to_lowest_ticket_fifo(self) -> None:
        memory_root = self._mkdtemp(prefix="execution_gate_exit_multi_fifo_")
        kwargs = _base_kwargs(memory_root)
        kwargs["decision"] = "WAIT"
        kwargs["controlled_mt5_readiness"] = {
            **dict(kwargs["controlled_mt5_readiness"]),
        ...
        mt5_stub = _MT5ExitMultiPositionFIFOConfirmedStub()
        kwargs["mt5_module"] = mt5_stub
        controlled_execution, _state, _paths = _run_controlled_mt5_live_execution(**kwargs)
        self.assertEqual(controlled_execution["order_result"]["status"], "accepted")
        self.assertTrue(controlled_execution["order_result"]["order_sent"])
        ...
        # Must resolve to ticket 940 (lowest/FIFO), not 950
        self.assertEqual(mt5_stub.send_calls, 1)
        sent_request = mt5_stub.sent_requests[0]
        self.assertEqual(sent_request["action"], "deal")
    ...
    def test_exit_multi_position_fails_closed_on_invalid_ticket(self) -> None:
        memory_root = self._mkdtemp(prefix="execution_gate_exit_multi_invalid_")
        kwargs = _base_kwargs(memory_root)
        kwargs["decision"] = "WAIT"
        kwargs["controlled_mt5_readiness"] = {
            **dict(kwargs["controlled_mt5_readiness"]),
        ...
        mt5_stub = _MT5ExitMultiPositionInvalidTicketStub()
        kwargs["mt5_module"] = mt5_stub
        controlled_execution, _state, _paths = _run_controlled_mt5_live_execution(**kwargs)
        ...
        self.assertIn("multi_position_ticket", rejection)

    def test_exit_multi_position_fails_closed_on_duplicate_tickets(self) -> None:
        memory_root = self._mkdtemp(prefix="execution_gate_exit_multi_dup_")
        kwargs = _base_kwargs(memory_root)
        kwargs["decision"] = "WAIT"
        kwargs["controlled_mt5_readiness"] = {
            **dict(kwargs["controlled_mt5_readiness"]),
        ...
        mt5_stub = _MT5ExitMultiPositionDuplicateTicketStub()
        kwargs["mt5_module"] = mt5_stub
        controlled_execution, _state, _paths = _run_controlled_mt5_live_execution(**kwargs)
        # No order should be sent — fail closed
        self.assertEqual(mt5_stub.send_calls, 0)
        rejection = controlled_execution.get("order_result", {}).get("rejection_reason", "")
        self.assertIn("exit_close_target_resolution_failed", rejection)
        self.assertIn("multi_position_duplicate_tickets", rejection)
