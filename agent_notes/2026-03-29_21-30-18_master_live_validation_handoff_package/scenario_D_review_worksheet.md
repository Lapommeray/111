## Scenario D Review Worksheet (copy-paste ready)

### 1) Run identity
- Scenario name: D — Spread-sensitive live behavior near threshold
- Run datetime (UTC):
...
### 8) Scenario D specific checks
- Spread above threshold?:
- Spread below threshold?:
- Spread blocker present only when expected?:
- Co-occurring open-position management/retry reasons preserved?:

### 9) Broker truth reconciliation
- Did broker truth eventually match expected state transition? (yes/no/partial):
- Evidence (position ticket/deal linkage/disappearance):
...
### 11) If failed — likely code re-entry point
- Suspected file/function from artifact evidence (exact path):
- Why this location is implicated (artifact evidence only):
