## Scenario E Review Worksheet (copy-paste ready)

### 1) Run identity
- Scenario name: E — Live macro pause during open-position management
- Run datetime (UTC):
...
- Macro pause state (`pause_trading`):
...
### 8) Macro-pause + open-position context coherence
- `macro_feed_unsafe_pause` present in blocker reasons? (yes/no):
- `open_position_exit_management:*` present in final reasons? (yes/no):
